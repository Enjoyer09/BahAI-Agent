...
export function useChatFlow(initialMessages = []) {
  ...
  // Normal Send
  const sendMessage = useCallback(async (text, attachments = [], parentMessageId = null) => {
  ...
  const regenerate = useCallback(async (assistantMessageId) => {
  ...
  const editAndResubmit = useCallback(async (userMessageId, newText) => {
