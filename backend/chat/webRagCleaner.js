...
function cleanWebAssistantResponse(text = '', isWebProduct = false) {
  ...
  // 1. Strip raw tool JSON invocations if leaked
  cleaned = cleaned.replace(/```(?:json)?\s*\{\s*"name"\s*:\s*"[^"]+"[\s\S]*?\}\s*```/gi, '');

  // 2. Strip internal agent monologues (e.g. "Axtarış aparıram...", "İndi Kapital bankı açıram...")
  const monologuePatterns = [
    /^(?:Pulunuzu depozitə qoşmaq üçün|Cari faiz dərəcələrini öyrənmək üçün|Axtarış aparıram|İndi birbaşa|Əvvəlcə Kapital Bank).*?(?:\n\n|\.\s+)/i,
